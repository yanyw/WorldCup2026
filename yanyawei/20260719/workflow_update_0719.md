...
决赛不能等同于普通淘汰赛。球队优化的是最终夺冠概率，而 1X2 和 O/U 合约只结算前 90 分钟；当前内部模型没有经过验证的决赛阶段、加时选择权或比分状态风险偏好变量。因此本场标记为 `PARTIAL_OOD_FINAL_STAGE`。

这不是硬性 `SKIP`：本场仍有同盘口、同为 90 分钟口径的机构与 Opta 直接证据，可以对合格盘口定价。但 P1/P2/P3 在本场只作诊断，不允许方向性移动执行概率：
...
该规则对 1X2 和 O/U 2.5 一致应用，不因某个方向看起来更有利而选择性启用。恢复内部权重的前提是决赛阶段模型通过事前定义的历史样本验证。

本次分类是在初版读取盘口后由适用域复核触发，必须标记为 post-hoc 方法纠错，不能计入任何策略改进的回测收益。后续版本应在抓取当期盘口前，先按 `IN_DOMAIN / PARTIAL_OOD / HARD_OOD` 完成赛事阶段登记并冻结处理规则。
...
决赛的 `PARTIAL_OOD_FINAL_STAGE` 覆盖上述一般规则：内部权重为 0，只保留诊断输出。
